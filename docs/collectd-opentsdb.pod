...
default C<write_tsdb> plugin which uses the legacy I<telnet>.
...
static tagging through a json object passed in Hostname).

Even if this plugin can partially work with older versions, using it with collectd >= 5.6 is strongly recommended
be able to use all functionalities, specially, I<JsonHostTag> and tag filtering.
...
Consequently choose your tags carefully, specially when also using B<write_opentsdb> Chains.

=item B<AutoFqdnFailback> B<true>|B<false>

If B<JsonHostTag> and B<AutoFqdnFallback> is set B<true> and if I<Hostname> failed to be parsed as
json, this plugin sets the B<fqdn> tag to I<Hostname> as a fallback.
...
These filtering rules relies on I<MetaData> option in I<set> Target.
...
With no filters, the metric name will be:
  ...
  <plugin>[.<pluginInstance>].<type>[.<typeInstance>].<ds_name>
...
Other options have no effect if B<tsdb_id> is set.
...
If B<tsdb_tag_pluginInstance> is set and I<String> is not empty, the I<plugin> 
part of the metric name is removed, and used as value of of tag I<String>

if I<String> is empty, it just removes the I<tsdb_tag_pluginInstance> part of the metric name,
no tag are set.

=item B<tsdb_tag_type> I<String>

If B<tsdb_tag_type> is set and I<String> is not empty, the I<plugin> 
part of the metric name is removed, and used as value of of tag I<String>

if I<String> is empty, it just removes the I<tsdb_tag_type> part of the metric name,
no tag are set.

=item B<tsdb_tag_typeInstance> I<String>

If B<tsdb_tag_typeInstance> is set and I<String> is not empty, the I<plugin> 
part of the metric name is removed, and used as value of of tag I<String>

if I<String> is empty, it just removes the I<tsdb_tag_typeInstance> part of the metric name,
no tag are set.

=item B<tsdb_tag_dsname>

If B<tsdb_tag_dsname> is set and I<String> is not empty, the I<plugin> 
part of the metric name is removed, and used as value of of tag I<String>

if I<String> is empty, it just removes the I<tsdb_tag_dsname> part of the metric name,
no tag are set.
